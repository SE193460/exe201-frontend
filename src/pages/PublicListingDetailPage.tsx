...
export default function PublicListingDetailPage() {
  ...
  function timeAgo(dateStr: string | null | undefined): string {
    ...
    if (seconds < 60) return t("Vài giây trước");
    const minutes = Math.floor(seconds / 60);
    if (minutes < 60) return t("{{minutes}} phút trước", { minutes });
    const hours = Math.floor(minutes / 60);
    if (hours < 24) return t("{{hours}} giờ trước", { hours });
    const days = Math.floor(hours / 24);
    if (days < 30) return t("{{days}} ngày trước", { days });
    const months = Math.floor(days / 30);
    if (months < 12) return t("{{months}} tháng trước", { months });
    return t("{{years}} năm trước", { years: Math.floor(months / 12) });
  }

  const REPORT_REASONS = [
    t("Tin có dấu hiệu lừa đảo"),
    t("Tin trùng lặp nội dung"),
    t("Không liên hệ được chủ tin đăng"),
    t("Thông tin không đúng thực tế (giá, diện tích, hình ảnh...)"),
    t("Lý do khác"),
  ];
  ...
  useEffect(() => {
    ...
    fetchPublicListingDetail(id)
      ...
      .catch(() => {
        setError(t("Không thể tải thông tin chi tiết phòng hoặc phòng chưa được kiểm duyệt."));
        setLoading(false);
      });
  }, [id]);
  ...
  const handleToggleSave = useCallback(async () => {
    ...
    try {
      ...
      alert(t("Không thể lưu bài đăng. Vui lòng đăng nhập."));
    } finally {
      setSaving(false);
    }
  }, [listing?.id]);
          ...
          <div className="mx-auto max-w-[1200px] px-4 md:px-6 py-3">
            <nav className="flex items-center gap-2 text-xs text-slate-500">
              <button onClick={() => navigate("/")} className="flex items-center gap-1 hover:text-[#a55b00] transition">
                <Home className="h-3.5 w-3.5" /> {t("Trang chủ")}
              </button>
              <ChevronRight className="h-3 w-3" />
              <button onClick={() => navigate("/listings")} className="hover:text-[#a55b00] transition">{t("Phòng trọ")}</button>
              <ChevronRight className="h-3 w-3" />
        ...
        {loading && (
          <div className="mx-auto max-w-[1200px] px-4 md:px-6 py-20 flex items-center justify-center">
            <div className="text-center">
              ...
              <p className="mt-3 text-sm text-slate-500">{t("Đang tải thông tin chi tiết...")}</p>
            </div>
          </div>
        )}
              ...
              <div className="space-y-6">
                ...
                <section className="rounded-[24px] bg-white px-5 py-6 shadow-[0_18px_45px_-35px_rgba(0,0,0,0.18)] border border-slate-200">
                  ...
                  {!listing.source && (
                    <div className="mt-5 flex flex-wrap gap-2">
                      <span className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-semibold border ${listing.smokingAllowed ? "bg-orange-50 border-orange-200 text-orange-700" : "bg-slate-50 border-slate-200 text-slate-500"}`}>
                        ...
                        {listing.smokingAllowed ? t("Cho phép hút thuốc") : t("Không hút thuốc")}
                      </span>
                      <span className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-semibold border ${listing.petAllowed ? "bg-orange-50 border-orange-200 text-orange-700" : "bg-slate-50 border-slate-200 text-slate-500"}`}>
                        ...
                        {listing.petAllowed ? t("Nuôi thú cưng") : t("Không nuôi thú cưng")}
                      </span>
                    </div>
                  )}
                  ...
                  <div className="mx-0 mt-6 rounded-[18px] border border-[#edd7be] bg-[#fff6eb] p-4 text-left text-xs space-y-1.5 text-[#9a6a3c] leading-relaxed">
                    ...
                    <p>{t("• Chỉ đặt cọc giữ chỗ khi đã xác thực danh tính chủ nhà và có thỏa thuận biên nhận rõ ràng.")}</p>
                    <p>{t("• Kiểm tra kỹ điều khoản hợp đồng trước khi thực hiện ký kết.")}</p>
                  </div>
                </section>
              </div>
              ...
              <aside className="space-y-4 lg:sticky lg:top-6">
                ...
                <div className="rounded-[24px] border border-[#edd7be] bg-[#fff6eb] p-4 shadow-[0_18px_45px_-35px_rgba(0,0,0,0.12)]">
                  <p className="text-xs font-bold text-[#8d4d08] flex items-center gap-1.5 mb-1.5">
                    <ShieldCheck className="h-4 w-4 text-[#a55b00]" /> {t("Mẹo an toàn")}
                    </p>
                    <p className="text-xs leading-5 text-[#9a6a3c]">{t("Không chuyển khoản đặt cọc khi chưa xem phòng trực tiếp và xác nhận thông tin chủ nhà.")}</p>
                </div>
              </aside>
            </div>

            <section className="mt-8 rounded-[24px] border border-slate-200 bg-white px-5 py-6 shadow-[0_18px_45px_-35px_rgba(0,0,0,0.14)]">
              <h3 className="text-lg font-bold text-slate-900 mb-4">{t("Phòng tương tự trong khu vực")}</h3>
              {similarListings.length > 0 ? (
                      ...
                      >
                        <div className="relative aspect-[4/3] bg-slate-100 overflow-hidden">
                          {thumbnail ? (
                            ...
                            <div className="flex h-full w-full items-center justify-center text-xs text-slate-400">{t("Chưa có ảnh")}</div>
                          )}
                          <span className="absolute left-3 top-3 rounded-full bg-white/90 px-2.5 py-1 text-[11px] font-bold text-[#a55b00] shadow-sm">
                            {item.rentPrice.toLocaleString("vi-VN")} {t("/ tháng")}
                          </span>
                        </div>
                        <div className="p-4">
                          <h4 className="line-clamp-2 text-sm font-semibold text-slate-900 group-hover:text-[#a55b00]">{item.title}</h4>
                          <p className="mt-2 line-clamp-1 text-xs text-slate-500">{location || t("Chưa cập nhật địa chỉ")}</p>
                        </div>
                      </button>
                    );
                  })}
                ...
                <p className="text-sm text-slate-500">{t("Tính năng đang được phát triển.")}</p>
              )}
            </section>
          </div>
        )}
            ...
            <div className="flex items-center justify-between border-b border-slate-700 px-4 py-3">
              ...
              <div className="flex items-center gap-2 rounded-xl bg-slate-800 p-1">
                <button onClick={() => setMediaModalTab("images")}
                  className={`rounded-lg px-4 py-2 text-sm font-semibold transition ${mediaModalTab === "images" ? "bg-white text-slate-900" : "text-slate-300 hover:bg-slate-700 hover:text-white"}`}>{t("Hình ảnh")}</button>
                <button onClick={() => setMediaModalTab("map")}
                  className={`rounded-lg px-4 py-2 text-sm font-semibold transition ${mediaModalTab === "map" ? "bg-white text-slate-900" : "text-slate-300 hover:bg-slate-700 hover:text-white"}`}>{t("Bản đồ")}</button>
              </div>
                    ...
                    <div className="flex gap-2 overflow-x-auto pb-1">
                      {listing.images.map((image, idx) => (
                        <button key={image.id} onClick={() => setSelectedImgIdx(idx)}
                          ...
                          <img src={resolveListingImageUrl(image.imageUrl)} alt={t("Ảnh phòng {{number}}", { number: idx + 1 })} className="h-full w-full object-cover" />
                        </button>
                      ))}
                    </div>
                  )}
          ...
          <div className="relative w-full max-w-[560px] max-h-[90vh] overflow-y-auto rounded-[24px] bg-white p-6 shadow-xl"
            ...
            {reportSuccess ? (
              ...
              <div className="mt-5 space-y-5">
                <div>
                  <p className="text-sm font-semibold text-slate-600 mb-3">{t("Lý do phản ánh:")}</p>
                ...
                <div>
                  <p className="text-sm font-semibold text-slate-600 mb-1">{t("Mô tả thêm")}</p>
                  <textarea value={reportDescription} onChange={(e) => setReportDescription(e.target.value)}
                    ...
                    placeholder={t("Nhập chi tiết lý do báo cáo...")} />
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <p className="text-sm font-semibold text-slate-600 mb-1">{t("Họ và tên")}</p>
                  ...
                  <div>
                    <p className="text-sm font-semibold text-slate-600 mb-1">{t("Số điện thoại")}</p>
                ...
                <div className="flex gap-3 pt-2">
                  ...
                  <button onClick={handleSubmitReport} disabled={!reportReason || submittingReport}
                    ...
                    {submittingReport ? t("Đang gửi...") : t("Gửi báo cáo")}
                  </button>
                </div>
              </div>
            )}
      ...
      {showShareModal && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center px-4 py-8" onClick={() => setShowShareModal(false)}>
          <div className="relative w-full max-w-[560px] rounded-[24px] bg-white p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <h2 className="text-lg font-bold text-slate-800 flex items-center gap-2"><Share2 className="h-5 w-5 text-blue-500" /> {t("Chia sẻ bài đăng")}</h2>
            <p className="text-sm text-slate-600 mt-3">{t("Sao chép đường dẫn hoặc chia sẻ tới ứng dụng khác.")}</p>
